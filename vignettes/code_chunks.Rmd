...
document.
    ...
    Observe in the output we have a numbered, floating figure. Because we
    ...
    pdf was saved in a file named "tmpout/p-chunk65-1.pdf". I chose the
...
display a matrix of figures "automatically", but I'll use a
\LaTeX\ structure to do this work.
...
several documents that output files into the same output folder.)

2. `split=TRUE` asks Sweave to keep each chunk's input and output
...
especially useful for tables. If a chunk's output is text with
\LaTeX\ markup, and we want that tex to go "into the document
immediately, right here," then we add the chunk argument
`results=tex`. However, it is also possible to use the "save something
for later" approach.
...
I might want that chunk in a different document. It is very convenient to
come along later and sort through the output in the tmpout folder. I might
