...
on the first page. It is more suitable for preparation of a report
to clients or a draft of a journal article. A report typically has
less (maybe no) code and almost never will it include ``raw
output'' from a computer program. A report includes tables
...
authors in a flexible way, providing one or more names that are
...
audience/client. If a ``paper'' must be
submitted, choose PDF. If the document needs numbered equations, cross
references, and ``floating'' tables and
figures, choose PDF. If the document is intended for a Webpage, then
choose HTML (although we prefer a PDF document via the Web in many
cases). 
