...
are not available via pure markdown. That puts us in a "gray area,"
...
just decorations on which authors waste their time. To test that
...
values.  The `key:value` pairs may make a difference at several
...
1. In-line math markup is dollar-sign bracketed, just as in
   \LaTeX\ itself. We write `$f_{ij} x_{i} \times y_{j}^2$` for
...
In our experimentation with R markdown, we have noticed another quirk.  In
a document intended for PDF, one writes `\LaTeX` to produce \LaTeX. On
...
a closer `]`. If we don't want their names, we insert a `-` sign. It is
